...
func (ws *WebSocketClient) handle(ctx context.Context, cfg types.Config, db *sql.DB) {
	...
	t08_00 := time.NewTicker(types.NextTime("09:07"))
	defer t08_00.Stop()
	t08_10 := time.NewTicker(types.NextTime("09:07"))
	defer t08_10.Stop()
	t09_00 := time.NewTicker(types.NextTime("09:07"))
	defer t09_00.Stop()
	t11_00 := time.NewTicker(types.NextTime("09:07"))
	defer t11_00.Stop()
	t18_00 := time.NewTicker(types.NextTime("09:07"))
	defer t18_00.Stop()
	t20_00 := time.NewTicker(types.NextTime("09:07"))
	defer t20_00.Stop()
	...
	for {
		...
		case <-t5m.C: //every 5 minutes
			...
			wg := &sync.WaitGroup{}
			wg.Go(func() { //start-of-work for duties
