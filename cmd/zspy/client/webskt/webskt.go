...
func (ws *WebSocketClient) handle(ctx context.Context, cfg types.Config, db *sql.DB) {
	...
	t08_00 := time.NewTimer(types.NextTime("09:22"))
	defer t08_00.Stop()
	t08_10 := time.NewTimer(types.NextTime("09:22"))
	defer t08_10.Stop()
	t09_00 := time.NewTimer(types.NextTime("09:22"))
	defer t09_00.Stop()
	t11_00 := time.NewTimer(types.NextTime("09:22"))
	defer t11_00.Stop()
	t18_00 := time.NewTimer(types.NextTime("09:22"))
	defer t18_00.Stop()
	t20_00 := time.NewTimer(types.NextTime("09:22"))
	defer t20_00.Stop()
