...
func Absents(ctx context.Context, db *sql.DB) ([]shared.Absent, error) {
	...
	rows, e := db.QueryContext(ctx, `
		...
		from (
			select coalesce(t.tabel_type, -1) as t1, 
				coalesce(tc.id, -1) as t2, 
				u.username as u, 
				coalesce(tt.caption, '') as c1, 
				coalesce(tc.caption, '') as c2, 
				coalesce(p.gender, 0) as g
			from tabel t
